...
export async function generateStaticParams() {
  return edebiyatAkimlari.map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({
  ...
  const akim = edebiyatAkimlari.find((a) => a.slug === slug);
  if (!akim) return {};
  return {
...
export default async function EdebiyatAkimiDetay({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const diger = edebiyatAkimlari.filter((a) => a.slug !== slug).slice(0, 3);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="flex items-center gap-2 text-amber-500 text-sm font-medium mb-8">
        ...
        <span>/</span>
        <Link href="/edebiyat-akimlari" className="hover:text-amber-400">Edebiyat Akımları</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <h2 className="text-xl font-bold text-white mb-4">✍️ Önemli Yazarlar</h2>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <h2 className="text-xl font-bold text-white mb-4">📚 Önemli Eserler</h2>
          <ul className="space-y-2">
      ...
      {/* Miras */}
      <div className="bg-gradient-to-br from-rose-900/20 to-gray-900 border border-rose-800/30 rounded-2xl p-8 mb-10">
      ...
      {/* Diğer Akımlar */}
      <div>
        <h2 className="text-xl font-bold text-white mb-4">✍️ Diğer Edebiyat Akımları</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {diger.map((a) => (
            <Link
              key={a.slug}
              href={`/edebiyat-akimlari/${a.slug}`}
              className="group bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-rose-700/40 hover:bg-gray-800 transition-all"
            >
              <div className="flex items-center gap-3">
                <span className="text-2xl">{a.emoji}</span>
                <div>
                  <div className="text-white font-medium text-sm group-hover:text-rose-400 transition-colors">
                    {a.ad}
                  </div>
