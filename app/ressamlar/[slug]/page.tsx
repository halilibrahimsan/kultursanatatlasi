...
export async function generateStaticParams() {
  return ressamlar.map((r) => ({ slug: r.slug }));
}
...
export async function generateMetadata({
  ...
  const ressam = ressamlar.find((r) => r.slug === slug);
  if (!ressam) return {};
  return {
    title: `${ressam.ad} | Kültür & Sanat Atlası`,
    description: ressam.ozet,
  };
}

export default async function RessamDetay({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const diger = ressamlar.filter((r) => r.slug !== slug).slice(0, 3);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="flex items-center gap-2 text-amber-500 text-sm font-medium mb-8">
        ...
        <span>/</span>
        <Link href="/ressamlar" className="hover:text-amber-400">Ressamlar</Link>
        <span>/</span>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8 mb-8">
        <div className="flex items-start gap-6">
          <div
            className={`w-24 h-24 rounded-3xl bg-gradient-to-br ${ressam.renk} flex items-center justify-center text-5xl flex-shrink-0`}
          >
            {ressam.emoji}
          </div>
          <div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 mb-6">
        ...
        <div className="space-y-4">
          {ressam.eserler.map((eser, i) => (
            <div
              key={i}
              className="border border-gray-800 rounded-xl p-5 hover:border-amber-700/40 transition-colors"
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="text-white font-semibold text-lg">{eser.ad}</h3>
                <span className="text-amber-400 text-sm font-medium whitespace-nowrap bg-amber-900/20 px-2 py-0.5 rounded-full">
                  {eser.yıl}
                </span>
              </div>
      ...
      {/* Diğer Ressamlar */}
      <div>
        <h2 className="text-xl font-bold text-white mb-4">👨‍🎨 Diğer Ressamlar</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {diger.map((r) => (
            <Link
              key={r.slug}
              href={`/ressamlar/${r.slug}`}
